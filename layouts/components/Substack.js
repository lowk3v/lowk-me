...
const Substack = ({ post, className }) => {
  return (
    <div className={className}>
      <div className="card">
        <ul className="flex items-center space-x-4">
          <li><Link className="text-primary" href={post.link} target='_blank'>
            {post.source}
          </Link></li>
        </ul>
        <p className="my-4">{post.pubDate}</p>
        <h2 className="h3 mb-2 font-normal">
          <Link href={post.link} className="block" target='_blank'>
            {post.title}
          </Link>
        </h2>
        <p className="mb-4"></p>
        {post.contentSnippet}
        <br/>
        <Link target='_blank' className="btn-link mt-7 inline-flex items-center hover:text-primary" href={post.link}>
          Continue Reading
          <svg
            className="ml-1"
            width="22"
